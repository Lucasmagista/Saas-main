-- SCRIPT ULTRA SEGURO - DESABILITA RLS E REMOVE TUDO
...
-- Remover TODAS as políticas existentes sem exceção
DO $$ 
DECLARE
    r RECORD;
BEGIN
    -- Remove todas as políticas das tabelas de permissões
    FOR r IN (SELECT schemaname, tablename, policyname FROM pg_policies 
              WHERE tablename IN ('permissions', 'roles', 'role_permissions')) 
    LOOP
...
ALTER TABLE IF EXISTS public.role_permissions DROP CONSTRAINT IF EXISTS role_permissions_role_id_fkey;
ALTER TABLE IF EXISTS public.role_permissions DROP CONSTRAINT IF EXISTS role_permissions_permission_id_fkey;
ALTER TABLE IF EXISTS public.user_roles DROP CONSTRAINT IF EXISTS user_roles_custom_role_id_fkey;

-- Remover coluna custom_role_id com CASCADE forçado
ALTER TABLE IF EXISTS public.user_roles DROP COLUMN IF EXISTS custom_role_id CASCADE;
...
DROP TYPE IF EXISTS public.system_module CASCADE;
...
-- Funções SQL (criar antes das políticas RLS)
...
-- Habilitar RLS nas novas tabelas DEPOIS de criar tudo
ALTER TABLE public.permissions ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.roles ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.role_permissions ENABLE ROW LEVEL SECURITY;

-- Políticas RLS SIMPLES (sem recursão) - criadas por último
