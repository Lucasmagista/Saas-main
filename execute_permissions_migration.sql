-- INSTRUÇÕES PARA EXECUTAR NO SUPABASE DASHBOARD
-- 1. Acesse: https://supabase.com/dashboard/project/qwddtvotgdzcswcvuqpw/sql/new
...
CREATE POLICY "Usuários podem visualizar permissões ativas" ON public.permissions
FOR SELECT USING (is_active = true);

-- Políticas RLS para roles
CREATE POLICY "Usuários podem visualizar cargos ativos" ON public.roles
FOR SELECT USING (is_active = true);

-- Políticas RLS para role_permissions
CREATE POLICY "Usuários podem visualizar associações de permissão" ON public.role_permissions
FOR SELECT USING (true);

-- Políticas para admins gerenciarem o sistema
...
CREATE POLICY "Admins podem gerenciar cargos" ON public.roles
FOR ALL USING (
  EXISTS (
    SELECT 1 FROM public.user_roles ur
    JOIN public.roles r ON ur.custom_role_id = r.id
...
CREATE POLICY "Admins podem gerenciar associações" ON public.role_permissions
FOR ALL USING (
  EXISTS (
    SELECT 1 FROM public.user_roles ur
    JOIN public.roles r ON ur.custom_role_id = r.id
