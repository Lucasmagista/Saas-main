...
// Limite global de requisições: 500 por IP a cada 15 minutos (aumentado para desenvolvimento)
...
// Limite específico para rota de autenticação (login/register): 10 requisições por 15 minutos
...
// Limite específico para operações de bots: 200 requisições por 15 minutos (aumentado para desenvolvimento)
...
app.use(async (req, res, next) => {
  if (["POST", "PUT", "DELETE"].includes(req.method)) {
    try {
      await logAudit({ req, action: req.method, resourceType: req.baseUrl, resourceId: req.body?.id, newValues: req.body });
    } catch (error) {
...
// Multi-sessions API - require authentication, rate limit e bind route
...
try {
  ...
  // Caso as rotas não existam (por exemplo, em teste ou ambiente de produção), apenas logamos
